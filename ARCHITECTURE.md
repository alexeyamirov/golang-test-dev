...
- **Назначение**: Прием TR181 данных из Apache Pulsar
- **Особенности**:
  - Потребляет сообщения из topic `tr181-device-data`
  - Сохранение всех метрик в PostgreSQL
  - Проверка условий для алертов
  - Публикация алертов в topic `alerts` для асинхронной обработки

### 3. Alert Processor (`services/alert-processor`)
- **Назначение**: Фоновая обработка алертов
- **Особенности**:
  - Потребляет алерты из Apache Pulsar topic `alerts`
  - Сохранение обработанных алертов в PostgreSQL
  - Поддержка задержки обработки (допускается запоздание)
  - CPU нагрузка для автоскейлинга
